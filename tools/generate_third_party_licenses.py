...
def render_archive(*, environment_label: str = "Windows") -> str:
    ...
    lines = [
        ...
        (
            f"used for the current {environment_label} release candidate. "
            "Regenerate it in the"
        ),
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=OUTPUT_PATH,
        help="archive output path (defaults to the committed Windows archive)",
    )
    parser.add_argument(
        "--platform-label",
        choices=("Linux", "Windows"),
        default="Windows",
        help="release environment recorded in the generated archive",
    )
    args = parser.parse_args()

    rendered = render_archive(environment_label=args.platform_label)
    if args.check:
        ...
        if current != rendered:
            if (
                sys.platform != "win32"
                and args.output == OUTPUT_PATH
                and args.platform_label == "Windows"
            ):
                print(
