...
> [!NOTE]
> NightScope is a released application. Source version 1.46.5 is prepared for
...
- Includes an offline celestial catalogue with 7,585 distinct deep-sky
  ...
  are explicitly marked as work in progress. Three remediation passes now
  replace formulaic historical guidance for 17 galaxies, 48 open clusters, and
  41 globular clusters. Sixteen cluster descriptions have also been rewritten
  as specific, source-backed content in all three languages. Per-target Home eligibility is
...
`dist/NightScope-v1.46.5-debian-12-x64.tar.gz` and its adjacent `.sha256`
