...
> [!NOTE]
> NightScope is a released application. Source version 1.46.4 is prepared for
...
- Includes an offline celestial catalogue with 7,585 distinct deep-sky
  ...
  are explicitly marked as work in progress. Two remediation passes now replace
  formulaic historical guidance for 17 galaxies and 48 open clusters, with ten
  cluster descriptions also rewritten as specific, source-backed content in all
  three languages. Per-target Home eligibility is
...
`dist/NightScope-v1.46.4-debian-12-x64.tar.gz` and its adjacent `.sha256`
