...
Lo step `1.14.11` aggiunge
`docs/NSOM_AOD_OPENAQ_CALIBRATION_AUDIT.md`, audit developer-only della formula
AOD/OpenAQ default-off. Conferma source precedence, fallback PM piu' debole,
neutralita' confidence e assenza di effetto runtime con flag spento; prima di
qualsiasi default-on restano da rivedere la scala assoluta del modifier aerosol
e la forma penalty-cap/transparency.
Lo step `1.14.12` calibra in modo mirato quella forma: il cap aerosol diventa
perdita massima di trasparenza (`penalty_cap / 100`) e il modifier compatibile
viene derivato da `target.score * transparency_loss`. Il flag resta spento di
default; resta da validare solo la scala aerosol prima di un eventuale
default-on.
Lo step `1.14.13` aggiunge
`docs/NSOM_AOD_OPENAQ_DEFAULT_ON_READINESS.md`: provider-quality, ownership
...
Lo step `1.14.15` aggiunge
`docs/NSOM_AOD_OPENAQ_REAL_PROVIDER_PROBE.md`: un probe developer-only usa
NASA Earthdata AOD e OpenAQ reali su Bologna, San Pedro de Atacama, New Delhi,
Mauna Kea e Addis Ababa. Il flag off resta neutro, il flag on sperimentale
...
Lo step `1.14.17` aggiunge
`docs/NSOM_AOD_OPENAQ_REAL_PROVIDER_READINESS_AUDIT.md`, audit developer-only
che rilegge il probe reale espanso senza chiamate di rete. La scala reale del
modifier viene accettata come coerente e modesta, ma il default-on resta
deferito per evidenza temporale insufficiente: nel report checked-in tutti gli
AOD utilizzabili sono `stale` e manca una seconda esecuzione provider in un
momento diverso. `experimental_aerosol_scoring` resta `False`.

La UI e il flusso principale sono considerati stabili per l'uso osservativo visuale. Le aree più sperimentali restano:

- dati VIIRS NASA, perché dipendono da connessione, credenziali Earthdata e disponibilità LAADS;
- dati NASA AOD sperimentali nella sezione Meteo `Trasparenza atmosferica`: sono informativi, dipendono da disponibilità MAIAC/cloud mask e alimentano punteggi solo nel path interno default-off, calibrato in 1.14.12 e auditato su provider reali in 1.14.17 ma ancora non abilitato di default;
