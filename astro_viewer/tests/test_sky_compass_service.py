...
def test_home_replaces_sky_map_with_sky_compass_without_timer() -> None:
    ...
    assert "Layout.preferredHeight: Math.max(140, homeEventContent.implicitHeight + 20)" in events_block
    assert "modelData.analysisBoundaryText" in events_block
    assert "Layout.alignment: Qt.AlignLeft | Qt.AlignTop" in events_block
