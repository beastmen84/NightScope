...
def test_active_legacy_or_hybrid_surfaces_remain_separate_from_dead_code_removal() -> None:
    ...
    assert "home_and_best_object_rerouted_sky_compass_pending" in active[
        "ObservationConditions prepared-object cache"
    ]["why_active"]
    assert "Sky Compass" in active["ObservationConditions prepared-object cache"]["recommended_handling"]
