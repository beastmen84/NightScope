...
def test_aod_openaq_calibration_audit_is_deterministic_strict_json_and_developer_only() -> None:
    ...
    assert first["readiness"]["formula_changed_by_audit"] is False
    assert first["readiness"]["weights_tuned_by_audit"] is False
...
def test_aod_openaq_calibration_audit_surfaces_default_on_review_items() -> None:
    ...
    assert set(data["default_on_blockers"]) == {
        "aerosol-score-scale-human-calibration",
        "penalty-cap-vs-transparency-shape",
    }
    assert review_items["aerosol-score-scale-human-calibration"]["blocks_default_on"] is True
    assert review_items["penalty-cap-vs-transparency-shape"]["blocks_default_on"] is True
...
def test_checked_in_aod_openaq_calibration_audit_report_matches_renderer() -> None:
    ...
    assert "aod_openaq_calibration_review_required" in text
    assert "Ready for default-on: `False`" in text
    assert "aerosol-score-scale-human-calibration" in text
