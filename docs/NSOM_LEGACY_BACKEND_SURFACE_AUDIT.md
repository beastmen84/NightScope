...
| --- | --- | --- | --- |
| Equipment recommendations | `active_legacy_or_hybrid` | `EquipmentService` still computes practical setup recommendations; `observer_capability_adapter.py` now provides shared ObserverCapability/Q_target projection while `docs/EQUIPMENT_SETUP_SCORE_OWNERSHIP_AUDIT.md` classifies the setup score components and `docs/EQUIPMENT_SETUP_SCORE_COMPONENT_BOUNDARY.md` verifies the component read-model parity. `docs/EQUIPMENT_NSOM_DEFAULT_OFF_PATH_POLICY_AUDIT.md` rejects a default-off replacement path for now and keeps Equipment setup-local with status `equipment_default_off_path_policy_set_setup_local`. `docs/EQUIPMENT_NSOM_MIGRATION_CLOSEOUT.md` closes Equipment as an NSOM-bounded setup-local service. Contract status: `equipment_setup_read_model_boundary_introduced`. Score ownership status: `equipment_setup_score_ownership_audited`. Component boundary status: `equipment_setup_score_component_boundary_introduced`. Default-off policy status: `equipment_default_off_path_policy_set_setup_local`. Closeout status: `equipment_nsom_migration_closed_setup_local`. | Review the 1.13.5 closeout for this setup-local service, then choose the next backend NSOM area or run an overall backend readiness audit. |
