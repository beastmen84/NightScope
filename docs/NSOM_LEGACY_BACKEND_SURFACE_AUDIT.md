...
| --- | --- | --- | --- |
| Equipment recommendations | `active_legacy_or_hybrid` | `EquipmentService` still computes practical setup recommendations; `observer_capability_adapter.py` now provides shared ObserverCapability/Q_target projection while `docs/EQUIPMENT_SETUP_SCORE_OWNERSHIP_AUDIT.md` classifies the setup score components before runtime replacement. Contract status: `equipment_setup_read_model_boundary_introduced`. Score ownership status: `equipment_setup_score_ownership_audited`. | Review the 1.13.2 ownership audit, then extract a runtime-neutral setup-score component read-model if parity can be preserved. |
