...
| --- | --- | --- | --- |
| Equipment recommendations | `active_legacy_or_hybrid` | `EquipmentService` still computes practical setup recommendations; `observer_capability_adapter.py` now provides shared ObserverCapability/Q_target projection while `docs/EQUIPMENT_NSOM_PRESENTER_CONTRACT_AUDIT.md` defines the setup payload/read-model contract now used before runtime replacement. Contract status: `equipment_setup_read_model_boundary_introduced`. | Review the 1.13.1 read-model boundary, then audit setup-score ownership for seeing, sky quality, target traits, fallback states and presentation-local selectionScore. |
