...
Current runtime status for `1.45.12`:
...
- `1.23.0` adds the 109 Caldwell targets through that identity contract. It
  expands the candidate pool without changing NSOM formulas or factor counts.
- `1.23.1` completes their descriptive/image seed coverage; this is presentation
  content only and does not enter NSOM scoring or ranking.
...
- `1.24.0` adds source-backed curiosities and licensed local survey cutouts as
  presentation content. Neither table supplies an NSOM factor, changes target
  identity or affects scoring, ranking and counts.
- `1.24.1` replaces Solar System presentation placeholders with source-backed
  NASA/JPL PIA images. Paths and image metadata change; NSOM inputs, formulas,
  target identity, scoring, ranking and counts remain unchanged.
...
- `1.26.0` adds a score-free filter recommendation to observing detail. Target
  preferences are matched only against filters in the active profile, with a
...
Runtime status in `1.45.12`: the AOD/PM, Moon and static-sky component caps are
