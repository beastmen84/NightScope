...
- [ ] Create and verify the public `v1.45.22` source tag referenced by the
  portable bundles and `SOURCE_CODE.md`.
- [x] Confirm source version, changelog, source-availability notices, and
  About/build metadata agree on `1.45.22`.
...
- [ ] Create the deterministic
  `NightScope-v1.45.22-debian-12-x64.tar.gz` and adjacent SHA-256 file.
...
- [ ] Publish the tarball and checksum together with the matching Windows ZIP
  in the public `v1.45.22` GitHub release, so stable-update notifications have
